...
function Container({ onClick }: { onClick?: () => void }) {
  return (
    <div 
      onClick={onClick}
      className="absolute bg-green-50 box-border content-stretch flex h-[321px] items-center justify-center left-[37px] p-[4px] rounded-[14px] top-[93px] w-[920px] cursor-pointer hover:opacity-90 transition-opacity" 
      data-name="Container"
    >
...
export default function BarcodeScannedScreen({ barcode, onTimeout, onClick }: BarcodeScannedScreenProps) {
  const handleClick = () => {
    if (onClick) {
      onClick();
    } else if (onTimeout) {
  ...
  return (
    <div className="relative h-[600.297px] w-[1024.5px]" data-name="BarcodeScannedScreen">
      <Container11 onClick={handleClick} />
    </div>
  );
}
